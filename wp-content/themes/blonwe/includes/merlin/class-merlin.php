...
class Merlin {
	...
	protected function license() {
        if(blonwe_get_registered_purchase_code()) {
	...
	protected function plugins() {
        if(empty(blonwe_get_registered_purchase_code())) {
            echo "<p>".esc_html__('Please verify the purchase code first.','blonwe')."</p>";
	...
	protected function content() {
		...
		<div class="merlin__content--transition merlin-blonwe--content">

			<img class="merlin-blonwe-logo" src="<?php echo get_template_directory_uri(); ?>/assets/img/logo-electronic.png"> 
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				...
				<div class="merlin__select-control-wrapper">
					<div class="klb-image-demo-select">
						<?php foreach ( $this->import_files as $index => $import_file ) : ?>
							<?php if(isset($import_file['import_img_url'])){ ?>
								<div class="klb-demo-item">
									<img src="<?php echo  $import_file['import_img_url'] ; ?>" width="150px" / value="<?php echo esc_attr( $index ); ?>">
									<h1><?php echo esc_html( $import_file['import_file_name'] ); ?></h1>
								</div>
							<?php } ?>
						<?php endforeach; ?>
					</div>
	...
	public function _ajax_activate_license() {
		...
		update_option( 'envato_purchase_code_48378751', $license_key );
