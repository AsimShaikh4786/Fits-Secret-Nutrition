...
/* Buy Now Single */
if(get_theme_mod('blonwe_shop_single_buy_now',0) == 1 || blonwe_ft() == 'buynow'){
...
if(get_theme_mod('blonwe_single_sticky_titles',0) == 1){
	require_once( __DIR__ . '/single-sticky-titles/single-sticky-titles.php' );
}

/* Sticky Single Cart */
if(get_theme_mod('blonwe_single_sticky_cart',0) == 1){
...
if(get_theme_mod('blonwe_shop_single_orderonwhatsapp', 0) == 1 || blonwe_ft() == 'orderonwhatsapp'){
	...
	function blonwe_order_on_whatsapp(){
		...
		echo '<a href="https://wa.me/'.esc_attr($number).'?text='.get_the_title().'" class="button button-primary">'.esc_html__('Order on WhatsApp','blonwe-core').'</a>';
		echo '</div>';
	}
}
...
function blonwe_product_box_stock_progress_bar($stockprogressbar){
	...
	if($managestock && $stock_quantity > 0) {
		...
		$output .= '<div class="available">Available:<strong>'.esc_html($stock_quantity).'</strong></div>';
		$output .= '<div class="sold">Sold: <strong>'.esc_html($total_sales).'</strong></div>';
